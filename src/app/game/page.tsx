...
function FannedCardBacks({
  ...
  const cardHeight = compact ? 50 : 80;
  const width = (compact ? 18 : 24) + displayCount * (compact ? 14 : 20);
  return (
    <div
    ...
    >
      {Array.from({ length: displayCount }).map((_, i) => {
        ...
        const lift = Math.abs(offset) * (compact ? 2 : 4);
        return (
          <div
            key={i}
            className="absolute bottom-0"
            style={{
              left: `calc(50% + ${offset * cardWidth}px)`,
...
function OpponentCard({
  ...
  player: Player;
  isActive: boolean;
  history: { cards: CardType[]; type: string }[];
  isMobile?: boolean;
  ...
  return (
    <div
      className={cn(
        "flex flex-col items-center gap-1 bg-gray-900/70 border rounded-xl transition-colors",
        isMobile ? "p-1.5 min-w-0" : "p-3 min-w-30",
        isActive
          ? "border-yellow-500/50 bg-yellow-500/5"
          : "border-gray-700/50",
      )}
    >
      <FannedCardBacks count={player.hand.length} isMobile={isMobile} />
      <p
        className={cn(
      ...
      <div className="flex gap-1 justify-center">
        {player.lockedOut && (
          <span
            className={`text-red-400 ${isMobile ? "text-[9px]" : "text-[10px]"}`}
          >
            {t("game.locked")}
          </span>
        )}
        {player.isOut && (
          <span
            className={`text-green-400 ${isMobile ? "text-[9px]" : "text-[10px]"}`}
          >
            {t("game.out")}
          </span>
        )}
        {isActive && (
          <span
            className={`text-yellow-400 animate-pulse ${isMobile ? "text-[9px]" : "text-[10px]"}`}
          >
            {t("game.thinking")}
          </span>
        )}
      ...
      <span
        className={`text-gray-500 ${isMobile ? "text-[9px]" : "text-[10px]"}`}
      >
      ...
      <div className="min-h-[calc(100vh-3.5rem)] flex flex-col bg-green-950">
        ...
        <div className="flex-1 relative mx-2 mt-2 mb-0">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div className="flex gap-1">
                    {currentPlay.cards.map((card, i) => {
                      const fromBottom =
                        currentPlay.playerId === humanPlayer?.id;
                      const animName = fromBottom
                        ? "card-fly-from-bottom"
                        : "card-fly-from-top";
                      return (
                        <div
                          key={card.id}
                          style={{
                            animation: `${animName} 0.5s ease-out ${i * 80}ms both`,
                          }}
                        >
          ...
          >
            ...
            >
              {humanPlayer?.hand.length} {t("common.cards")}
            </p>
            <PlayedCards
              records={
                isMobile
                  ? gameHistory
                      .filter((r) => r.playerId === humanPlayer?.id)
                      .slice(-1)
                  : gameHistory.filter((r) => r.playerId === humanPlayer?.id)
              }
              fanned={!isMobile}
            />
          </div>
        ...
        <div
          className={`bg-black/50 backdrop-blur border-t border-green-900/50 ${compact ? "p-1" : "p-3"}`}
        >
        ...
        {phase === "game-over" && winDetail && (
          <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-2 overflow-y-auto">
            <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 max-w-sm w-full my-8">
              <div className="text-center mb-4">
                <h2 className="text-xl font-bold text-white mb-1">
                  {winner === "player" ? t("game.youWon") : t("game.youLost")}
                </h2>
                <p className="text-gray-400 text-sm">
                  {t(winDetail.summaryKey, winDetail.summaryParams)}
                </p>
              </div>
              ...
              <div className="bg-gray-800/30 rounded-lg p-3 mb-4 space-y-3">
                <p className="text-xs text-gray-500 uppercase tracking-wider">
                  {t("game.winnerCards")}
                </p>
                {(() => {
                  ...
                  if (!winnerPlayer)
                    return <p className="text-gray-500 text-xs">—</p>;
